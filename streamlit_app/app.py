...
def calculate_weighted_average(offers_df, trade_type):
    ...
    if 'price' not in filtered_offers.columns or 'tradableQuantity' not in filtered_offers.columns:
        st.warning("Missing 'price' or 'tradableQuantity' column in offers data.")
        return None
    filtered_offers['price'] = pd.to_numeric(filtered_offers['price'], errors='coerce')
    filtered_offers['tradableQuantity'] = pd.to_numeric(filtered_offers['tradableQuantity'], errors='coerce')
    
    # Drop rows where conversion failed
    filtered_offers.dropna(subset=['price', 'tradableQuantity'], inplace=True)

    if filtered_offers['tradableQuantity'].sum() == 0:
        return None
    
    weighted_average = (filtered_offers['price'] * filtered_offers['tradableQuantity']).sum() / filtered_offers['tradableQuantity'].sum()
...
if pairs_data:
    ...
    cryptos = sorted(list(set(p['crypto'] for p in pairs_data)))
    trade_types = sorted(list(set(p['trade_type'] for p in pairs_data)))
    ...
    if st.button("Fetch Offers"):
        with st.spinner("Fetching offers..."):
            ...
            if offers:
                ...
                buy_orders['cumulative_amount'] = buy_orders['tradableQuantity'].cumsum()
                # Calculate cumulative volume for sell orders
                sell_orders['cumulative_amount'] = sell_orders['tradableQuantity'].cumsum()
